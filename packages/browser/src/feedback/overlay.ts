...
export class FeedbackOverlay {
  ...
  private pausedDesignMode = false;
  ...
  mount(): void {
    if (this.host) return;
    const dm = (globalThis as unknown as { __agentBridgeDesignMode?: { disable: () => void; enable: () => void; status?: () => { enabled: boolean } } }).__agentBridgeDesignMode;
    if (dm) {
      if (dm.status?.()?.enabled) {
        this.pausedDesignMode = true;
      }
      dm.disable();
    }
  ...
  unmount(): void {
    ...
    this.root = null;
    this.canvas = null;
    if (this.pausedDesignMode) {
      this.pausedDesignMode = false;
      const dm = (globalThis as unknown as { __agentBridgeDesignMode?: { enable: () => void } }).__agentBridgeDesignMode;
      dm?.enable();
    }
  }
  ...
  private handleAction(event: Event): void {
    ...
    if (action === 'collapse') {
      ...
      const removed = this.controller.undoLastMark();
      if (removed) this.undoStack.push(removed);
      this.paintCanvas();
    } else if (action === 'redo') {
      const mark = this.undoStack.pop();
      if (mark) this.controller.addMark(mark);
      this.paintCanvas();
    } else if (action === 'clear') {
      this.controller.clearCurrentMarks();
      this.paintCanvas();
      ...
      if (!id) return;
      const comment = window.prompt('Comment on acceptance') ?? undefined;
      this.controller.acceptSuggestion(id, comment);
    } else if (action === 'reject-suggestion') {
      const id = target.dataset.suggestionId;
      if (!id) return;
      const comment = window.prompt('Reason for rejection') ?? undefined;
      this.controller.rejectSuggestion(id, comment);
    } else if (action === 'comment-suggestion') {
      const id = target.dataset.suggestionId;
      if (!id) return;
      const comment = window.prompt('Comment') ?? '';
      if (comment) this.controller.commentOnSuggestion(id, comment);
    }
  }
